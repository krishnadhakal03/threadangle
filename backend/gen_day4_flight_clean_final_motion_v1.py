...
RUN_ID = "day4_flight_clean_final_motion_v1"
...
(async () => {{
  ...
  try {{
    ...
    await page.mouse.move(360, 560, {{ steps: 16 }});
    await page.waitForTimeout(180);
    await box.click({{ timeout: 5000 }});
    await page.waitForTimeout(220);
    await page.keyboard.type('CLT to MCO', {{ delay: 42 }});
    await page.waitForTimeout(360);
    await page.keyboard.type(' Google Flights', {{ delay: 34 }});
...
def make_scene_clips(capture: dict) -> list[Path]:
    ...
    for idx, (src, crop, zoom) in enumerate(scene_sources, start=1):
        ...
        if idx == 2:
            make_browser_clip(search_motion, dst, DURATIONS[idx - 1], "crop=900:1600:90:120,scale=1080:1920", start_offset=0.20)
        elif idx == 3:
            make_browser_clip(flights_motion, dst, DURATIONS[idx - 1], "crop=900:1600:90:145,scale=1080:1920", start_offset=1.65)
        else:
    ...
    LOG_PATH.write_text(
        "\n".join(
            [
                ...
                "scene2_typing_confirmation=confirmed: Playwright typed 'CLT to MCO Google Flights' and pressed Enter during recorded clip",
