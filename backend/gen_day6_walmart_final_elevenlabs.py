#!/usr/bin/env python3
"""Generate Day6 Walmart+ receipts proof final premium voice pass.

Picture-lock constraints:
- use ElevenLabs narration only for this approved final pass
- no RunwayML
- no visual redesign
...
RUN_ID = "day6_walmart_receipts_proof_FINAL_elevenlabs"
...
PICTURE_LOCK_PATH = ASSETS_DIR / "day6_walmart_receipts_proof_v3b_audiofix.mp4"
ELEVENLABS_VOICE_ID = "pNInz6obpgDQGcFmaJgB"
ELEVENLABS_VOICE_NAME = "Adam"
ELEVENLABS_MODEL_ID = "eleven_multilingual_v2"
ELEVENLABS_SPEED = 1.0
PREMIUM_AUDIO_TEMPO = 1.15
NARRATION_PLAN = [
    ...
    {"start": 14.30, "caption": "Walmart+ for $12.95", "tts": "Walmart Plus for twelve ninety-five"},
    {"start": 15.72, "caption": "covered delivery,", "tts": "covered delivery,"},
    {"start": 16.88, "caption": "Peacock, and cashback.", "tts": "Peacock, and cashback."},
...
CREDIT_USAGE_PATH = TEMP_DIR / f"{RUN_ID}_elevenlabs_chars.txt"
...
def elevenlabs_phrase(text: str, out: Path) -> int:
    ...
    if response.status_code != 200:
        raise RuntimeError(f"ElevenLabs TTS failed: HTTP {response.status_code} {response.text[:300]}")
...
def make_audio() -> tuple[Path, str, float]:
    ...
    for idx, item in enumerate(NARRATION_PLAN, start=1):
        raw = WORK_DIR / f"elevenlabs_phrase{idx:02d}.wav"
        character_count += elevenlabs_phrase(str(item["tts"]), raw)
    ...
    for idx, (item, raw) in enumerate(zip(NARRATION_PLAN, raw_parts), start=1):
        ...
        clean = WORK_DIR / f"elevenlabs_phrase{idx:02d}_timed.wav"
        run(
            [
                "ffmpeg", "-y", "-i", str(raw), "-af",
                f"atempo={PREMIUM_AUDIO_TEMPO:.3f},aresample=48000,aformat=sample_fmts=s16:channel_layouts=stereo",
                "-ar", "48000", "-ac", "2", str(clean),
            ],
            f"time premium phrase {idx}",
        )
    ...
    CREDIT_USAGE_PATH.write_text(str(character_count), encoding="utf-8")
    return AUDIO_PATH, f"ElevenLabs {ELEVENLABS_VOICE_NAME} ({ELEVENLABS_VOICE_ID}) model={ELEVENLABS_MODEL_ID}", wpm
...
def main() -> None:
    if not PICTURE_LOCK_PATH.exists():
        raise FileNotFoundError(PICTURE_LOCK_PATH)
    ...
    picture_lock_probe = probe_json(PICTURE_LOCK_PATH)
    final_duration = float(probe.get("format", {}).get("duration") or 0)
    lock_duration = float(picture_lock_probe.get("format", {}).get("duration") or 0)
    duration_delta = abs(final_duration - lock_duration)
    credit_chars = CREDIT_USAGE_PATH.read_text(encoding="utf-8").strip()
    ...
    LOG_PATH.write_text(
        "\n".join(
            [
                ...
                f"elevenlabs_voice_used={ELEVENLABS_VOICE_NAME} ({ELEVENLABS_VOICE_ID})",
                f"elevenlabs_credit_usage_estimate={credit_chars}_characters",
                f"average_narration_pacing_estimate={narration_wpm:.1f}_wpm_active_speech",
                f"picture_lock_runtime={lock_duration:.6f}",
                f"final_runtime={final_duration:.6f}",
                f"runtime_drift_seconds={duration_delta:.6f}",
                ...
                f"audio_pacing_confirmation=confirmed: ElevenLabs premium narration aligned to v3b timing intent with uniform tempo factor {PREMIUM_AUDIO_TEMPO:.2f}; intentional micro-pauses around the $42 visual beat, $53-to-$12.95 proof beat, $480 payoff, and CTA",
                "money_pronunciation_confirmation=confirmed: TTS text uses spoken forms including fifty three, twelve ninety-five, and four hundred eighty",
                ...
                "paid_credit_scope_confirmation=confirmed: ElevenLabs approved for this pass only; no RunwayML and no other paid APIs used",
