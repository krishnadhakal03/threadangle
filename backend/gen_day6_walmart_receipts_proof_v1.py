...
RUN_ID = "day6_walmart_receipts_proof_v1"
...
SOURCE_TEXTS = [
    "I was spending $42 a month just getting to Walmart. I don't have a car. I thought that was just my life.",
    "That was leaving my account every month.",
    "So I typed my exact situation into AI and asked one thing.",
    "That replaced about $53 a month with about $13. No Uber. No Peacock bill.",
    "About $480 a year. Same Walmart. Zero Uber rides.",
    "Comment WALMART and I'll send you exactly what I typed could save you $40 every month.",
]

TTS_TEXTS = [
    "I was spending forty two dollars a month just getting to Walmart. I don't have a car. I thought that was just my life.",
    "That was leaving my account every month.",
    "So I typed my exact situation into AI and asked one thing.",
    "That replaced about fifty three dollars a month with about thirteen. No Uber. No Peacock bill.",
    "About four hundred eighty dollars a year. Same Walmart. Zero Uber rides.",
...
def draw_scene1(t: float) -> Image.Image:
    ...
    src = cover_image(SCREENSHOTS["lyft"], 1.07)
    paste_pan(img, src, t, DURATIONS[0], 0.5, 0.05, 0.40)
...
def draw_scene2(t: float) -> Image.Image:
    ...
    draw = ImageDraw.Draw(img)
    src = cover_image(SCREENSHOTS["peacock"], 0.82)
    ghost = Image.new("RGB", (WIDTH, HEIGHT), (248, 250, 252))
    paste_pan(ghost, src, t, DURATIONS[1], 0.5, 0.15, 0.35)
...
def draw_scene5(t: float) -> Image.Image:
    if t < 2.35:
        ...
        draw = ImageDraw.Draw(img)
        center(draw, (55, 590, 1025, 805), "SAVE ABOUT\n$480 A YEAR", font(92, True), (5, 15, 30), 10)
        center(draw, (90, 860, 990, 940), "Without changing\nwhere I shop", font(48, True), (71, 85, 105), 6)
        center(draw, (90, 1080, 990, 1140), "+ 5% cashback upside", font(36, True), (5, 150, 105))
        return img
    img = Image.new("RGB", (WIDTH, HEIGHT), (255, 255, 255))
    src = cover_image(SCREENSHOTS["savings"], 0.86)
    paste_pan(img, src, t - 2.35, 1.65, 0.5, 0.15, 0.32)
    return img
...
.answer{{opacity:0;transform:translateY(16px);margin-top:28px;background:#ecfdf5;border:1px solid #bbf7d0;border-radius:18px;padding:26px;font-size:35px;line-height:1.32;font-weight:900;color:#065f46;transition:all .25s ease-out}}
...
<div class="answer" id="answer">Walmart+ covers grocery delivery and includes Peacock. Estimate: about $12.95/month before any cashback.</div>
</div></div>
<script>
...
setTimeout(() => {{ const a=document.getElementById('answer'); a.style.opacity=1; a.style.transform='translateY(0)'; }}, 3950);
</script></body></html>""",
        encoding="utf-8",
    )
...
"""
    ...
    for idx, (text, dur) in enumerate(zip(SOURCE_TEXTS, DURATIONS)):
        ...
        for cidx, chunk in enumerate(chunks):
            events.append(f"Dialogue: 0,{ts(start + cidx * step)},{ts(start + (cidx + 1) * step)},Cap,,0,0,0,,{{\\an2\\pos(540,{y_positions[idx]})}}{chunk}")
        cursor += dur
...
def make_review_artifacts() -> tuple[Path, list[Path]]:
    ...
    offsets = [2.0, 2.0, 4.1, 5.4, 1.6, 2.8]
    frames: list[Path] = []
    cursor = 0.0
