...
RUN_ID = "day4_flight_clean_final_ai_overview_v1"
...
def make_ai_overview_clip(src: Path, dst: Path, duration: float) -> None:
    crop = f"crop=900:1600:90:95,scale={WIDTH}:{HEIGHT},fps={FPS},format=yuv420p"
    run(
        [
            "ffmpeg",
            "-y",
            ...
            "-filter_complex",
            (
                f"[0:v]trim=start=0.20:end=2.15,setpts=(PTS-STARTPTS)/1.85,{crop}[typed];"
                f"[0:v]trim=start=9.55:end=12.50,setpts=(PTS-STARTPTS)*0.98,{crop}[overview];"
...
def write_captions() -> int:
    ...
    for dur in DURATIONS:
        ...
        cursor += dur
    y_positions = [1450, 1490, 1510, 1500, 1585, 1470]
