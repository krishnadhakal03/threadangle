...
RUN_ID = "day4_google_flights_monster_v2"
...
async function recordLocal(name, html, action) {{
  ...
  await browser.close();
  return {{ name, webm: outWebm, real_browser: false, privacy_ok: true, reason: '' }};
}}
...
async function recordFlights(name, variant) {{
  ...
  fs.mkdirSync(videoDir, {{ recursive: true }});
  let context;
  let status = {{ name, webm: path.join(RAW_DIR, `${{RUN_ID}}_${{name}}.webm`), real_browser: true, privacy_ok: false, reason: '' }};
  try {{
...
(async () => {{
  ...
  const results = [];
  results.push(await recordLocal('scene1_hook_card', localHtml('Flight Hook', `
  ...
  results.push(await recordLocal('scene4_local_ai_prompt', localHtml('Local AI Assistant', `
  ...
  results.push(await recordLocal('scene5_proof_comparison', localHtml('Proof Comparison', `
...
def main() -> None:
    ...
    for idx, item in enumerate(results, start=1):
        ...
        logs.append(
            ...
            f"privacy_ok={item.get('privacy_ok')} reason={item.get('reason') or 'ok'} "
