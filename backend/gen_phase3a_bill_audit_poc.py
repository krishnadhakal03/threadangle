...
RUN_ID = "phase3a_bill_audit_poc_v1"
...
FPS = 30
FRAME_FPS = 15
DURATIONS = [4.0, 6.0, 7.0, 7.0, 7.0, 5.0]
FINAL_DURATION = sum(DURATIONS)

SOURCE_TEXTS = [
    "I asked AI to audit a monthly bill.",
    "First, I listed the recurring charges.",
    "Then I asked AI to flag overlaps and savings.",
    "It found three possible savings.",
    "That is fifty three dollars a month, or six hundred thirty six dollars a year.",
    "Comment AUDIT and I'll send the prompt.",
]
...
AI_LINES = [
    ("Cancel overlap", "save $18/month"),
    ("Renegotiate phone bill", "save $30/month"),
    ("Downgrade storage", "save $5/month"),
]
...
def frame_scene(scene: int, progress: float) -> Image.Image:
    ...
    if scene == 1:
        ...
        center_text(draw, (100, 430, 980, 820), "AI FOUND MONEY LEAKS\nIN THIS MONTHLY BILL", font(78, True), (255, 255, 255), 16)
        center_text(draw, (100, 900, 980, 1005), "Example audit", font(48, True), (82, 196, 255))
        ...
        typed = PROMPT[: int(len(PROMPT) * min(1.0, progress / 0.78))]
        lines = wrap_words(draw, typed + ("|" if progress < 0.82 else ""), font(42, True), box[2] - box[0] - 70, 4)
        ty = box[1] + 92
        for line in lines:
        ...
        draw = ImageDraw.Draw(img)
        draw.rounded_rectangle((58, 190, 1022, 1425), radius=32, fill=(255, 255, 255), outline=(82, 196, 255), width=10)
        center_text(draw, (100, 260, 980, 360), "Example audit", font(56, True), (15, 23, 42))
        center_text(draw, (100, 455, 980, 560), "Possible savings:", font(54, True), (71, 85, 105))
        center_text(draw, (100, 605, 980, 785), "$53/month", font(120, True), (5, 150, 105))
        draw.rounded_rectangle((100, 900, 980, 1250), radius=28, fill=(10, 16, 28))
        center_text(draw, (120, 945, 960, 1125), "$636/year", font(128, True), (255, 255, 255))
        center_text(draw, (120, 1140, 960, 1225), "Potential annual savings", font(42, True), (82, 196, 255))

    elif scene == 6:
        img = Image.new("RGB", (WIDTH, HEIGHT), (8, 13, 24))
        draw = ImageDraw.Draw(img)
        draw.rounded_rectangle((72, 310, 1008, 1160), radius=30, fill=(14, 24, 39), outline=(82, 196, 255), width=8)
        center_text(draw, (95, 395, 985, 680), "COMMENT\nAUDIT", font(124, True), (255, 255, 255), 12)
        center_text(draw, (95, 750, 985, 890), "FOR THE PROMPT", font(70, True), (82, 196, 255))
        center_text(draw, (95, 985, 985, 1065), "Save before paying", font(44, True), (255, 255, 255))
...
def write_captions() -> int:
    ...
    y_positions = [1490, 1550, 1535, 1535, 1660, 1510]
    ...
    LOG_PATH.write_text(
        "\n".join(
            [
                ...
                "motion=confirmed: cursor/typing in scenes 2-3; AI output reveals line by line in scene 4",
